...
const replit = require("quick.replit")
...
module.exports = async function () {
  console.clear()
  const spinner = createSpinner(`Bot processing by \u001b[32;1m'OnlyDemon.#4447\u001b[0m`).start({ color: 'green' })
  ...
  client.login(token).then(() => {
    ...
    spinner.error({ text: 'Invalid Bot Token or Disabled intents' })
  })
  ...
  client.on("ready", async () => {
    ...
    spinner.success({ text: `Logged in as ${client.user.tag} (${client.user.id})`, color: 'bgBlack' })
    app.get('/',(r, s) => {
       s.send({message:"Bot by 'OnlyDemon.#4447",youtube_channel:"https://www.youtube.com/0slicky"})
      }).post('/',async(r, s) => {
        s.send({
          message:"Bot by 'OnlyDemon.#4447",  youtube_channel:"https://www.youtube.com/0slicky"
        })
        if(await config.has(`uptime`) != true){
          console.log("\u001b[32m✔ \u001b[0mUptime has been done successfully")
          await config.set(`uptime`,true)
        }
      })
     .get("/invite", (req, res) => res.status(301).redirect(bot_invite_link))
     .listen(3000)
    console.log("\u001b[32m▣ \u001b[38;5;37;1mMusic\u001b[0m Bot By \u001b[38;5;38;1m'OnlyDemon.#4447\u001b[0m ")
  ...
  client.on("interactionCreate", async (interaction) => {
    if (interaction.type == 2) {
      ...
      if (interaction.commandName == "help") {
        let embeds = [{
          author: { name: "قائمة المساعدة", url: `https://www.youtube.com/0slicky` },
          description: `يدعم بوت الموسيقى أكثر من 1400 موقع يمكنك تشغيل من خلاله الاغاني تم برمجة وصناعة البوت بكامل وجميع الحقوق لدي [سليك](https://www.youtube.com/0slicky)`,
          fields: [{
            name: `أوامر البوت`,
            value: `${commands.map(x => `> \`/${x.name}\` → ${x.description}`).join("\n")}`
          }],
          color: 0x00ff00,
          footer: { text: `Bot By 'OnlyDemon.#4447` }
        }]
